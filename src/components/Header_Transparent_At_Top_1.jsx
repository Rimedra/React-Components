...
export default function Header_Transparent_At_Top_1() {
  ...
  return (
    <>
      <header className={` fixed top-0 z-50 transition-all duration-300 w-full  ${scrolled ? 'bg-white shadow-md  text-[#617C58] ' : 'text-[#617C58] bg-gray-100/80'}`}>
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 items-center justify-between h-16 grid grid-cols-12 gap-4">
          {/* Logo / Brand */}
          <div className="col-span-6 lg:col-span-2">
            <a href="#" className="text-xl font-bold  hover:text-[#c08763]">
              Logo or Text
            </a>
          </div>
          {/* Nav – Big Screen */}
          <nav className="hidden lg:flex col-span-7 justify-center">
            {navItems.map((item) => (
              <a
                key={item.label}
                href={item.href}
                className="px-4 py-2 rounded-md text-md font-bold  transition"
              >
                {item.label}
              </a>
            ))}
